...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
	...
	RMSE_list = []
	reg_list = np.random.uniform(0.0, 150.0, 300)
	reg_list.sort()
  #checked solution
...
def grad_descent(X_train, y_train, X_val, y_val, reg = 0.0, \
  ...
  #checked the gradient value from solutions
	b_grad = 1.
  ...
  #forgot to upgrade so had to check solution
		W -= lr_W * W_grad
		b -= lr_b * b_grad
...
if __name__ == '__main__':
  ...
  #added these after checking solution
  ...
  # was not working with (np.ones(y_train), X_train), so checked solution to use np.ones_like
